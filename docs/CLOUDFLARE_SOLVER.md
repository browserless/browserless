...
**Why this changed:** The previous implementation used `DOM.getDocument(depth=-1, pierce=true)` on the page session. While `DOM.getDocument` itself is safe (C++ layer), the old detection path also triggered Phase 1's `Runtime.callFunctionOn` through ReplaySession's tainted WS. `Target.getTargets` is simpler and guaranteed safe — it operates at the browser level, never touching any page's V8 context.

**Pydoll's approach:** `_find_cloudflare_shadow_root` uses `DOM.getDocument(depth=-1, pierce=True)` → walks shadow roots → checks `inner_html` for `challenges.cloudflare.com`. This works for pydoll because its page WS has a clean V8 context.
...
| `turnstile` | `Target.getTargets` detection → `solveTurnstile` → click → token appears → `cf.solved` |
...
|---------|-------|----------|
| `DOM.getDocument` | Page or OOPIF | C++ layer, bypasses V8 entirely |
| `DOM.getDocument(pierce:true)` | Page or OOPIF | Same C++ layer — `pierce` walks shadow roots in C++, no V8 |
...
|---------|-------|---------------|
| `Runtime.evaluate` | Page session | Executes in the PAGE's V8 context. CF's WASM monitors V8 evaluation events. Even `document.title` triggers detection. |
