...
export const isLegacyAPIEndpoint = (pathname: string): boolean => {
  const legacyEndpoints = ['/content', '/pdf', '/screenshot', '/scrape', '/performance'];
  
  return legacyEndpoints.some(endpoint => 
    pathname === endpoint || 
    (pathname.endsWith(endpoint) && 
     !pathname.includes('/edge/') && 
     !pathname.includes('/chrome/') && 
     !pathname.includes('/chromium/') && 
     !pathname.includes('/firefox/') && 
     !pathname.includes('/webkit/'))
  );
};
