...
export const beforeRequest: (
  args: IBeforeHookRequest,
) => boolean = fs.existsSync(beforeHookPath)
  ? require(beforeHookPath)
  : () => true;

export const afterRequest: (
  args: IAfterHookResponse,
) => boolean = fs.existsSync(afterHookPath)
  ? require(afterHookPath)
  : () => true;

export const browserHook: (
  opts: IBrowserHook,
) => Promise<boolean> = fs.existsSync(browserSetupPath)
