...
describe('Edge WebSocket API', function () {
  ...
  it('rejects websocket requests', async () => {
    ...
    const didError = await puppeteer
      .connect({
        browserWSEndpoint: `ws://localhost:3000/edge?token=bad`,
      })
      .then(() => false)
      .catch(() => true);
  ...
  it.skip('runs with ignored arguments', async () => {
    ...
    const success = await puppeteer
      .connect({
        browserWSEndpoint: `ws://localhost:3000/edge?token=browserless&launch=${JSON.stringify(
          args,
        )}`,
      })
      .then(async (b) => {
  ...
  it('runs with job-based timeouts', async () => {
    ...
    const browser = await puppeteer.connect({
      browserWSEndpoint: `ws://localhost:3000/edge?timeout=1000&token=browserless`,
    });
  ...
  it('fails requests without tokens', async () => {
    ...
    return puppeteer
      .connect({ browserWSEndpoint: `ws://localhost:3000/edge` })
      .catch((error: Error) => {
        const results = metrics.get();
        expect(results.successful).to.equal(0);
        expect(results.rejected).to.equal(0);
