...
class SchemaResolver {
    ...
    constructor(root: any, { subSchemas, refineType, refineSchema, strictMode, useDefaults, extensions = [] }: SchemaResolverOptions = {}) {
        this.root = root;
    ...
    resolveOneOf(schema: JSONSchema, ancestors: string[]): any {
        ...
        return this.joi.alternatives(schema.oneOf!.map((subSchema: any) => this.resolve(subSchema, ancestors))).match('one');
    }

    resolveAnyOf(schema: JSONSchema, ancestors: string[]): any {
        Hoek.assert(isArray(schema.anyOf), 'Expected anyOf to be an array.');

        return this.joi.alternatives(schema.anyOf!.map((subSchema: any) => this.resolve(subSchema, ancestors))).match('any');
    }

    resolveAllOf(schema: JSONSchema, ancestors: string[]): any {
        Hoek.assert(isArray(schema.allOf), 'Expected allOf to be an array.');

        return this.joi.alternatives(schema.allOf!.map((subSchema: any) => this.resolve(subSchema, ancestors))).match('all');
    }
