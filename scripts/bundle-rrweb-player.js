...
(async () => {
  ...
  // Bundle rrweb-player as IIFE for browser
  await build({
    entryPoints: [join(rootDir, 'node_modules/rrweb-player/dist/rrweb-player.js')],
    bundle: true,
    format: 'iife',
  ...
  // Read the CSS file
  const cssPath = join(rootDir, 'node_modules/rrweb-player/dist/style.css');
...
// Bundled from rrweb-player for player HTML page
